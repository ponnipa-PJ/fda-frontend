          ...
          <table class="table table-bordered">
            ...
            <tbody>
              <tr v-for="(t, td) in taxdatalist" :key="td+1">
                ...
                <th style="text-align: right">{{ numberWithCommas(taxsum[td]) }}</th>
                </tr>
              ...
              <tr>
                ...
                <th v-for="(hea, i) in overalltax" class="table-secondary"
                  :key="i+100" style="text-align: center; font-weight: bold">{{numberWithCommas(hea)}}</th>
                <!-- <th
...
// console.log(this.tbsize);
    ...
    loadchartOptions() {
      this.chartOptions = {
        ...
        colors: ["#77B6EA", "#EC7C30","#AC7C30","#AC7830"],
        dataLabels: {
          enabled: true,
        },
        stroke: {
    ...
    for (let ss = 0; ss < this.detail.length; ss++) {
      console.log(this.detail[ss].brand_name);
      console.log(this.detail[ss].degree);
      console.log(this.detail[ss].num);
      // for (let ds = 0; ds < this.detail[ss].data.length; ds++) {
      //   console.log(this.detail[ss].data[ds]);
  ...
  mounted() {
    ...
    var taxdata = [{ m: 10, n: 'ตุลาคม', y:year-1}, { m: 11, n: 'พฤศจิกายน', y:year-1 }, { m: 12, n: 'ธันวาคม', y:year-1 }, { m: 1, n: 'มกราคม', y:year }, { m: 2, n: 'กุมภาพันธ์' , y:year}, { m: 3, n: 'มีนาคม', y:year }, { m: 4, n: 'เมษายน' , y:year}, { m: 5, n: 'พฤษภาคม', y:year }, { m: 6, n: 'มิถุนายน' , y:year}]
...
this.net_tax = net_tax
    ...
    MergeLiquorReportService.getDetailbyfactory_code(this.excise_no).then(
            (res) => {
              // console.log(res.data);
              this.detail = res.data;
this.taxdatalist = taxdata
      for (let taxd = 0; taxd < this.taxdatalist.length; taxd++) { 
                  
                  for (let s = 0; s < this.detail.length; s++) {
          MergeLiquorReportService.getaxlist(this.excise_no,this.detail[s].brand_name,this.detail[s].degree,this.detail[s].size,this.taxdatalist[taxd].m,this.taxdatalist[taxd].y).then(
            (res) => {
              console.log(res.data[0].num);
              console.log(res.data[0].vol);
              var num = res.data[0].num
              var vol = res.data[0].vol
              num = num == null ? 0 : num;
              vol = vol == null ? 0 : vol;
              list.push(num)
              list.push(vol)
...
// console.log(list.length);
              var no = parseInt(this.detail.length*2)
              // console.log(no);
              if (list.length == no) {

              // taxdatalist[taxd].data = list
                this.taxdatalist[taxd].data = list
                list = []
              }
          });
         }
        }
        for (let tx = 0; tx < taxdata.length; tx++) {
              MergeLiquorReportService.getnettax(this.excise_no,taxdata[tx].m,taxdata[tx].y).then(
            (taxs) => {
...
this.overalltax.push(overallvol)
                console.log(this.overalltax.length);
                console.log(parseInt(this.detail.length*2));
