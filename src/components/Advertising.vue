  ...
  <div class="container">
      ...
      <tbody v-if="l.keyword">
        ...
        <tr v-for="(k, i) in l.keyword" :key="i">
          ...
          <td ><button @click="savetorulr_based(k,1)" type="submit" class="mb-3 btn btn-success">
            <i class="fa fa-check"></i>
    </button><button @click="savetorulr_based(k,9)" type="submit" class="mb-3 btn btn-danger">
...
export default {
  ...
  methods: {
    savetorule(answer){  
      ...
      for (let d = 0; d < this.arrList.length; d++) {
      // console.log(datasplit[d]);
      DictService.getdicts('',this.arrList[d]).then((res)=>{
        
        if (res.data.length > 0) {
          // console.log(res.data[0].id);
         iddata.push(res.data[0].id)
        }
        // console.log(d+1 , this.arrList.length);
        
      if (d+1 == this.arrList.length) {
        var sql =`SELECT * FROM rule_based WHERE `
    ...
    savetorulr_based(keyword,answer){
      this.arrList = []
      console.log(answer);
...
//       });
  ...
  mounted() {
    ProductsService.getproductkeyword(1).then(async (res)=>{
        // this.list = res.data
        console.log(res.data);
      ...
      // // console.log('http://127.0.0.1:5000/checkkeyword?name=' + desc);   
      axios.get('http://127.0.0.1:5000/worktokendesc?text='+res.data[l].desc).then((desc) => {
        // console.log(res.data);
        res.data[l].desctext  = desc.data 
       axios.get('http://127.0.0.1:5000/checkkeyword?name=' + res.data[l].desc).then((keys) => {
